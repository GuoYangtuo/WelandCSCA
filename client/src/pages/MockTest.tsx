...
const MockTest: React.FC = () => {
  ...
  const getScoreLevel = (percentage: number) => {
    if (percentage >= 90) return { level: '优秀', class: 'excellent', emoji: '🏆' };
    if (percentage >= 80) return { level: '良好', class: 'good', emoji: '🎯' };
    if (percentage >= 60) return { level: '及格', class: 'pass', emoji: '💪' };
    return { level: '需加强', class: 'fail', emoji: '📚' };
  };
